        ...
        return flightRepository
                ...
                .flatMap(exists -> {
                    ...
                    if (flight.getDepartureTime().isBefore(LocalDateTime.now())) {
                        return Mono.error(new ResponseStatusException(HttpStatus.BAD_REQUEST, "Departure time cannot be in the past"));
                    }
                    if (flight.getTotalSeats() <= 0) {
                        return Mono.error(new ResponseStatusException(HttpStatus.BAD_REQUEST, "Seats must be greater than zero"));
                    }
                    return flightRepository.save(flight);
                })
                .onErrorMap(e -> {
